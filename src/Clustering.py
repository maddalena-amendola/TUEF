...
def update_test(data_dir, clusters, exps):
    ...
    a_test.to_csv(data_dir + 'answers_test.csv.gz', compression='gzip', index=False)#, index=True, index_label = 'Index')
    
    print('\tTets size:', len(test))
...
def extract_topics(data_dir, struc_dir, n_features, min_k, max_k):
    ...
    update_test(data_dir, clusters, experts)
