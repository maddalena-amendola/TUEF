...
def join_layer_experts(data, l2r_df, exp_df, exp_layer_ans, hit = False):
    ...
    for i, (qid, acc, topics, answerers) in enumerate(tqdm(list(zip(l2r_df.Id, l2r_df.AcceptedAnswerer, l2r_df.Topic, l2r_df.Answerers)))):
        ...
        if (hit and acc in df.Expert.values) or not hit:     
            ...
            df_filtered = df_agg[df_agg['Expert'].isin([int(elem) for elem in answerers])]

            if(len(df_filtered)<20):
                to_select = 20-len(df_filtered)
                selected = df_agg[~df_agg['Expert'].isin([int(elem) for elem in answerers])].sample(n=to_select, random_state=1)
                selected = pd.concat([df_filtered, selected], ignore_index=True, axis=0)
                
                selected.index = np.arange(len(selected))  

            #-----------------------------------------------------------------
            ratio = []
            for e in selected.Expert:
...
def parse_arguments():
    ...
    parser.add_argument('--mink', required=False, type=int, default=2)
    parser.add_argument('--maxk', required=False, type=int, default=10)
    parser.add_argument('--features', required=False, type=int, default=10)
    parser.add_argument('--minaccans', required=False, type=int, default=3)
    parser.add_argument('--probability', required=False, type=float, default=0.001)
    parser.add_argument('--restart', required=False, type=int, default=5)
    parser.add_argument('--steps', required=False, type=int, default=10)
