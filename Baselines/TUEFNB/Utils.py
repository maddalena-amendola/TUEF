...
def intra_layer(data, layer):
    ...
    df_a = df_a.drop(['FreqIndexTag', 'FreqIndexText', 'ScoreIndexTag', 'ScoreIndexText' ], axis = 1)
    df_a.index = range(len(df_a))
    
    df_conc = pd.concat([df_a])
    agg_dict = {'MethodNetwork': 'sum', 
                'RW': 'min'}
    for col in df_conc.columns.difference(['Expert', 'Weight', 'MethodNetwork']):
        agg_dict[col] = 'first'
        ...
        for method in elem.keys():
            for layer in elem.get(method).keys():
                for expert in elem.get(method).get(layer).get('Weights'):
                    ...
                    elem[method][layer]['Weights'][expert]['Closeness'] = node['closeness'] 
                    elem[method][layer]['Weights'][expert]['PageRank'] = node['pagerank'] 
                    elem[method][layer]['Weights'][expert]['Eigenvector'] = node['eigen']
